...
class MF_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        thermal = self.read_t_image(name, 'ndvi', 'TIF')
        re = self.read_t_image(name, 're', 'TIF')
        ...
        re = np.asarray(PIL.Image.fromarray(re).resize((self.input_w, self.input_h)), dtype=np.float32)[np.newaxis, :]
        ...
        return torch.tensor(image), torch.tensor(thermal), torch.tensor(re), torch.tensor(label), name
...
class MF_dataset_ms(Dataset):
    ...
    def __getitem__(self, index):
        ...
        thermal = self.read_n_image(name, 'ndvi', 'TIF')
        re = self.read_n_image(name, 're', 'TIF')
        label = self.read_image(name, 'labels', 'png')
